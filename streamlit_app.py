...
# -----------------------------
# Section 1: Travel Planning Assistant (Fixed Stars & Clean UI)
# -----------------------------
if section == "Travel Planning Assistant":
    st.markdown(
        "<div style='text-align:center; font-size:36px; font-weight:800; margin-bottom:20px;'>🌍 Travel Planning Assistant</div>",
        unsafe_allow_html=True
    )
    ...
    if 'recommendations' in st.session_state:
        ...
        st.markdown(
            f"<div style='font-size:26px; font-weight:700; margin-top:20px;'>🏖️ Top {len(recs)} Destination Recommendations for You:</div>",
            unsafe_allow_html=True
        )

        for idx, row in recs.iterrows():
            stars_html = display_stars_html(row['score'])  # get HTML for stars
